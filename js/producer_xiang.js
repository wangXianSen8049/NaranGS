...
/*会填数据 内容*/
$(function(){
	var url=" http://192.168.3.29:8082/web/third/platform/farm/detailsHasCheck/"+getUrlParam("farmId");
	$.ajax({
        type: "post",
        dataType: "json",
        url: url,
        success: function (data){
			...
			if(detail==null){
				...
				$.each(detail.otherReports,function(n,value){
			 	...
			 	pingjia(detail.checks);//评价回填
			}
        }
    });
});
/*回填评语*/
function pingjia(arr){
	$.each(arr,function(name,value){
        $("."+value.checkType).text(value.description);
        /**
         * 其他报告书 信息产品 多个存在问题
         * 需要根据结尾符号 分割字符串
         */
        if(value.checkType=="other_tuijian"){
        	var arr = value.description.split(',');
        	$(".other_tuijian").each(function(i,n){
        		$(n).text(arr[i]);
			});
        }
        if(value.checkType=="other_falv"){
        	var arr = value.description.split(',');
        	$(".other_falv").each(function(i,n){
        		$(n).text(arr[i]);
			});
        }
        if(value.checkType=="q_baogao"){
        	var arr = value.description.split(',');
        	$(".q_baogao").each(function(i,n){
        		$(n).text(arr[i]);
			});
        }
       	if(value.checkType=="c_mingcheng"){
         	var arr = value.description.split(',');
        	$(".c_mingcheng").each(function(i,n){
        		$(n).text(arr[i]);
			});
        }
       	if(value.checkType=="c_shangbiao"){
         	var arr = value.description.split(',');
        	$(".c_shangbiao").each(function(i,n){
        		$(n).text(arr[i]);
			});
        }
       	if(value.checkType=="c_guige"){
         	var arr = value.description.split(',');
        	$(".c_guige").each(function(i,n){
        		$(n).text(arr[i]);
			});
        }
       	if(value.checkType=="c_chanliang"){
         	var arr = value.description.split(',');
        	$(".c_chanliang").each(function(i,n){
        		$(n).text(arr[i]);
			});
        }
       	if(value.checkType=="c_baogao"){
         	var arr = value.description.split(',');
        	$(".c_baogao").each(function(i,n){
        		$(n).text(arr[i]);
			});
        }
       	if(value.checkType=="c_zhengshu"){
         	var arr = value.description.split(',');
        	$(".c_zhengshu").each(function(i,n){
        		$(n).text(arr[i]);
			});
        }
       	if(value.checkType=="c_xiangqing"){
         	var arr = value.description.split(',');
        	$(".c_xiangqing").each(function(i,n){
        		$(n).text(arr[i]);
			});
        }
    });
...
function otherLegalOpinions(val){
	...
	var $list=$(
		...
		"</div>"+
		"<p class='ping other_falv' contenteditable='true' style='color: #FF4F65;'></p>");
	$(".falv").after($list);
	var form_="#"+form;
	var upfile_="#"+upfile;
	var dian_="#"+dian;
...
function otherRecommendations(val){
	...
	var $list=$(
		...
		"</div>"+
		"<p class='ping other_tuijian' contenteditable='true' style='color: #FF4F65;'></p>");
	$(".tuijian").after($list);
	var form_="#"+form;
	var upfile_="#"+upfile;
	var dian_="#"+dian;
...
function product_dom(value){
	...
	var $li = $("<div class='out pro'>"+
					"<div class='li'>"+
						...
						"<i>*</i>"+
					"</div>"+
					"<p class='ping c_mingcheng' contenteditable='true' style='color: #FF4F65;'></p>"+
					"<div class='li comm4'>"+
						...
						"<input type='hidden' id="+yin_shangbiao+" class='shangbiao'  name="+yin_shangbiao+" value='' />"+
			  			"<span>商标：</span>"+
				  	"</div>"+
				  	"<p class='ping c_shangbiao' contenteditable='true' style='color: #FF4F65;'></p>"+
			  		"<div class='li comm4'>"+
						...
						"<i>*</i>"+
					"</div>"+
					"<p class='ping c_guige' contenteditable='true' style='color: #FF4F65;'></p>"+
					"<div class='li comm'>"+
						"<input type='text' id="+estimatedYield+" class='estimatedYield' value="+value.estimatedYield+">"+
						"<span>产品预估产量：</span>"+
						"<i>*</i>"+
					"</div>"+
					"<p class='ping c_chanliang' contenteditable='true' style='color: #FF4F65;'></p>"+
					"<div class='li'>"+
						...
						"<i>*</i>"+
					"</div>"+
					"<p class='ping c_baogao' contenteditable='true' style='color: #FF4F65;'></p>"+
					"<div class='li comm4'>"+
				  	...
				  	"</div>"+
				  	"<p class='ping c_zhengshu' contenteditable='true' style='color: #FF4F65;'></p>"+
					"<div class='li comm1'>"+
					...
					"</div>"+
					"<p class='ping c_xiangqing' contenteditable='true' style='color: #FF4F65;'></p>"+
				"</div>");
	var $parent = $(".product");
	$parent.append($li);
...
function other_dom(value){
	...
	var $li = $("<div class='li huanj'>"+
				...
				"</div>"+
				"<p class='ping q_baogao' contenteditable='true' style='color: #FF4F65;'></p>");
	var $parent = $(".huanjing");
	var form_ ="#"+form; 
	var upfile_ ="#"+upfile; 
	var path_ ="#"+path; 
